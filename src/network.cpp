...
bool connectWifiWithFeedback() {
  ...
  for (uint8_t attempt = 1; attempt <= kSystemConfig.maxWifiAttempts; ++attempt) {
    unsigned long startAttempt = millis();
    while (millis() - startAttempt < kSystemConfig.wifiConnectTimeoutMs) {
      int status = wifiMulti.run();
      if (status == WL_CONNECTED) {
        return true;
      }
      uint8_t percent = static_cast<uint8_t>((attempt * 100) / kSystemConfig.maxWifiAttempts);
      TftDisplay::drawBootScreen("Connecting WiFi", percent);
      delay(kSystemConfig.wifiRetryDelayMs);
    }
  }
...
void initOta() {
  ArduinoOTA.setHostname(kSystemConfig.mdnsHost);
